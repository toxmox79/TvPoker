...
function newRoom() {
  ...
  rooms[code] = {
    ...
    playerContributions: {},  // track cumulative bets per player for side pots
    // ---
    handNum: 0,
    settings: {
...
function createSidePots(room) {
  const pots = [];
  const levels = [...new Set(room.players.map(p=>p.totalBet).filter(v=>v>0))].sort((a,b)=>a-b);
  let prev = 0;
  ...
  for (const pot of pots) {
    ...
    if (best) {
      ...
      for (let i=0; i<potWinners.length; i++) {
        ...
        allWinners.push({
          id:potWinners[i].id, name:potWinners[i].name, avatar:potWinners[i].avatar,
          handName:potWinners[i].handName, amount:share
        });
      }
    }
  }
